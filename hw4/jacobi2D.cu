...
__global__ 
void jacobi2D_step_kernel(double* u_new, double* u, double* temp, double h, long N) {
  ...
  if ( (i > 0) && (i < N+1) && (j > 0) && (j < N+1) ) {
  	u_new[i+j*(N+2)] = 0.25*(h*h 
			...
			+ u[(i+1)+(j)*(N+2)] + u[(i)+(j+1)*(N+2)] );
    __syncthreads();
  	temp[i+j*(N+2)] = u[i+j*(N+2)];
  	u[i+j*(N+2)] = u_new[i+j*(N+2)];
  	u_new[i+j*(N+2)] = temp[i+j*(N+2)];
  }
}
...
void jacobi2D_cuda(double* u_new_d, double* u_d, double* temp_d, double h, long N, long max_iter) {
  dim3 GridDim(N/THREADS_PER_BLOCK, N/THREADS_PER_BLOCK);
  dim3 BlockDim(THREADS_PER_BLOCK, THREADS_PER_BLOCK);
  for (long iter = 0; iter < max_iter; iter++) {
  	jacobi2D_step_kernel<<<GridDim, BlockDim>>>(u_new_d,  u_d, temp_d, h, N);
	}
}
...
int main(void) {
  ...
  jacobi2D_cuda(u_new_d, u_d, temp_d, h, N, max_iter);
  ...
  residual = calc_residual(u_new, h, N);
  printf("residual = %f\n",residual);
